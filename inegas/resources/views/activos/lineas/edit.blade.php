...
@section('content')
    <div class="row">
        <div class="col-md-12">
            <div class="card ">
                ...
                <form method="POST" action="{{url('act/lineas/'.$linea -> id)}}" autocomplete="off">
                    <div class="card-body ">
                        {{method_field('patch')}}
                        {{csrf_field()}}
            ...
            <div class="card ">
                ...
                <div class="card-body ">
                    <form method="GET" action="{{url('act/lineas/'.$linea -> id.'/edit')}}" autocomplete="off">
                        <div class="form-group form-file-upload form-file-multiple">
                            <div class="input-group">
                                ...
                                <span class="input-group-btn">
                                        ...
                                        <button type="button" class="btn btn-fab btn-round btn-primary" onclick="modalCreate('{{url('act/lineas/'.$linea -> id.'/grupo')}}')">
                            ...
                            <tbody>
                                @foreach($grupos as $grupo)
                                    <tr>
                                        ...
                                        <td>{{$grupo -> nombre}}</td>
                                        <td class="text-right ">
                                            <button type="button" class="btn btn-outline-primary btn-sm" onclick="modalEdit('{{$grupo -> nombre}}','{{url('act/lineas/'.$linea -> id.'/grupo/'.$grupo -> id)}}' )">
                                                <i class="fa fa-pen"></i>
                                            </button>
                                            <button type="button" class="btn btn-outline-primary btn-sm" onclick="eliminarGrupo('{{$grupo -> nombre}}','{{url('act/lineas/'.$linea -> id.'/grupo/'.$grupo -> id)}}')">
    ...
    <!-- end row -->
    @include('activos.lineas.grupos.modal')
    @include('modal')
    @push('scripts')
        <script>
